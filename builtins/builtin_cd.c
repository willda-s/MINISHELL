...
/*   Updated: 2025/09/25 23:55:19 by cafabre          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*handle_cd_without_path(t_data *data, char *path, char *old_pwd)
{
...
int	builtin_cd(t_exec *exec, t_data *data)
{
	...
	if (!exec->cmd[1])
	{
		path = handle_cd_without_path(data, path, old_pwd);
		if (!path)
			return (EXIT_FAILURE);
	}
	else
